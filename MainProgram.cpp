...
public:
    ...
    if(index< 0 || index >= size()) throw std:: out_of_range(" Box index out of range");
...
template<>
int describe(const std::string& value){
...
public:
    ...
    void set(int index, const T& value) {
        // replace
        if(index < 0 || index >= N)
            throw std::out_of_range("FixedArray indec out of range");
        data_[index] = value;
    }
